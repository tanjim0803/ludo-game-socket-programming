...
const move = new Audio("../sounds/Untitled_Project_V1.mp3");
...
socket.on("your_turn", () => {
  ...
  if (playersMove === getPlayerIndex(currentPlayerColor)) {
    gameloop();
  }
});

function getPlayerIndex(color) {
  switch (color) {
    case "red":
      return 1;
    case "green":
...
function gettingData() {
  ...
  // Fallback to URL params (singleplayer)
    ...
    .catch(() => {
      try {
        ...
        if (
          ...
          error.message === "token is not iterable"
        ) {
          playersMove++;
...
function tokenFloat(playersMove) {
  ...
  token.forEach((t) => {
    if (
      t.parentElement.classList.contains("disks") ||
      t.parentElement.classList.contains("tokenHome")
    ) {
      ...
      switch (playersMove) {
        case 1:
          var matchingClass = [...t.parentNode.classList].find((className) =>
            className.startsWith("redPath")
          );
          var cureentSpot = parseInt(matchingClass.substring(7));
          break;
        case 2:
          var matchingClass = [...t.parentNode.classList].find((className) =>
            className.startsWith("greenPath")
          );
          var cureentSpot = parseInt(matchingClass.substring(9));
          break;
        case 3:
          var matchingClass = [...t.parentNode.classList].find((className) =>
            className.startsWith("yellowPath")
          );
          var cureentSpot = parseInt(matchingClass.substring(10));
          break;
        case 4:
          var matchingClass = [...t.parentNode.classList].find((className) =>
            className.startsWith("bluePath")
          );
...
function moveToken() {
  ...
  switch (playersMove) {
    case 1:
      var matchingClass = [...this.parentNode.classList].find((className) =>
        className.startsWith("redPath")
      );
  ...
  function movingToken(diceOutcome) {
    ...
    if (diceOutcome == 0) {
      ...
      if (
        path.classList.contains("tokenHome") &&
        path.querySelectorAll("img").length < 4
      ) {
        homeChance = true;
        update();
      } else {
...
function killToken(Token, path) {
  if (
    !(
      path.classList.contains("star-place") ||
      path.classList.contains("tokenStart")
    )
  ) {
    ...
    tokensInPath.forEach((t) => {
      ...
      if (tokenName != otherTokenName) {
        ...
        var elementsToCheck = path.querySelectorAll(
          `[name ="${otherTokenName}"]`
        );
  ...
  switch (playerWons) {
    case 1:
      if (!won1st) {
        ...
        showSpot.innerHTML =
          '<img src="Won1st.png" alt="1st won" width="100%" >';
        won.play();
        won1st = true;
      }
    ...
    case 2:
      if (!won2nd) {
        showSpot.style.display = "flex";
        showSpot.innerHTML =
          '<img src="Won2nd.png" alt="1st won" width="100%" >';
        won2nd = true;
        won.play();
      }
      break;
    case 3:
      if (!won3rd) {
        showSpot.style.display = "flex";
        showSpot.innerHTML =
          '<img src="Won3rd.png" alt="1st won" width="100%" >';
        won3rd = true;
        won.play();
      }
      break;
